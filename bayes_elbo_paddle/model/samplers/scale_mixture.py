...
class ScaleMixture(nn.Layer):
    ...
    def __init__(self, pi: float, sigma1: float, sigma2: float) -> None:
        ...
        self.normal1 = paddle.distribution.Normal(0, sigma1) # 有问题，需要shape
        self.normal2 = paddle.distribution.Normal(0, sigma2)
    ...
    def log_prior(self, w: Tensor) -> Tensor:
        ...
        """
        likelihood_n1 = paddle.exp(self.normal1.log_prob(w))
        likelihood_n2 = paddle.exp(self.normal2.log_prob(w))

        p_scalemixture = self.pi * likelihood_n1 + (1 - self.pi) * likelihood_n2
        log_prob = paddle.log(p_scalemixture).sum()
